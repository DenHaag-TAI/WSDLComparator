...
public abstract class AbstractSchemaElementComparator<T extends SchemaElement> extends AbstractComparator<T> {
	...
	protected ChangedResult analyzeChangedObject(ChangedResult result) {
		DocumentationComparator documentationComparator = new DocumentationComparator(
				getOldObject().getDocumentation(), getNewObject().getDocumentation());
		result.addChild(documentationComparator.compare());
		...
		try {
			...
			for (Map.Entry<QName, String> newOtherAttribute : getNewObject().getOtherAttributes().entrySet()) {
				if (!contains(oldOtherAttributesQNames, newOtherAttribute.getKey())) {
					result.addChild(new AddedResult(Compatibility.BREAKS, "Attribute added", newOtherAttribute
							.getValue()));
				}
			}
