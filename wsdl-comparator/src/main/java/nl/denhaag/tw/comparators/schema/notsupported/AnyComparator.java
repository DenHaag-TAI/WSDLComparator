...
import org.ow2.easywsdl.schema.org.w3._2001.xmlschema.Any;

public class AnyComparator extends AbstractComparator<Any> {
	private boolean baseType;
	public AnyComparator(Any oldObject, Any newObject) {
		super(oldObject, newObject);
	}

	public AnyComparator(Any oldObject, Any newObject,  boolean baseType) {
		super(oldObject, newObject);
		this.baseType = baseType;
	}
	@Override
	protected ChangedResult analyzeChangedObject(ChangedResult result) {
		result.init(Compatibility.UNSUPPORTED, "Any is not supported");
	...
	protected AddedResult analyzeAddedObject(AddedResult result) {
		...
		result.init(Compatibility.BREAKS, "Any");
		return result;
	}
